...
class Cow:
    def __init__(self):
        ...
        self.flag = True
        self.par = (0, 0)
        # pg.draw.polygon(self.surf, (255, 255, 0), [(0, 0), (self.rect.x, 0), (self.rect.x, self.rect.y), (0,
        # self.rect.y)], 0)
...
class Potato(Herb):
    ...
    def grow(self):
        if self.iter_counter > 720 and self.thirst and self.stage < 6:
            if self.thirst:
                match self.stage:
                    case 1:
...
class Wheat(Herb):
    ...
    def grow(self):
        if self.iter_counter > 550 and self.thirst and self.stage < 6:
            if self.thirst:
                match self.stage:
                    case 1:
...
class Item:
    def __init__(self, path, slot, count):
        self.path = path
        self.count = count
        self.slot = slot
...
with open("data.json", "r", encoding="utf-8") as file:
    data = json.load(file)
print(data)
inventory = Inventory(data["inventory_picked"])
wheat = []
potatoes = []
sprinklers = []
cords = []
...
iters = 0
counter = 0
background_mark = data["background_mark"]
nature_iters = 0
shop = Shop()
money = Money(data["slot_10"])
...
slot_6 = shop.Slot((WIDTH / 2 + WIDTH / 4), (HEIGHT - HEIGHT / 4), "Images/Money/Coin_icon.png",
                   "Images/Sprinkler/Sprinkler_icon.png", 6)
...
inventory_use = []
animals = []
player = Player(data["pos_x"], data["pos_y"])
animals.append(Cow())
animals.append(Cow())
animals.append(Cow())
wheat_seed = Seed("Images/Wheat/Wheat_seed.png", 0, data["slot_1"])
potato_seed = Seed("Images/Potato/Potato_seed_item.png", 2, data["slot_3"])
...
wheat_item = Item("Images/Wheat/Wheat_produce.png", 1, data["slot_2"])
potato_item = Item("Images/Potato/Potato_produce.png", 3, data["slot_4"])
milk_item = Item("Images/Milk/Milk_produce.png", 4, data["slot_5"])
sprinkler_item = Item("Images/Sprinkler/Sprinkler_item.png", 5, data["slot_6"])
...
while flag_play:
    ...
    for event in pg.event.get():
        if event.type == pg.QUIT:
            wheat_coordinates = []
            for elem in wheat:
                wheat_coordinates.append((elem.coordinate_x(), elem.coordinate_y()))
            potato_coordinates = []
            for elem in potatoes:
                potato_coordinates.append((elem.coordinate_x(), elem.coordinate_y()))
            sprinklers_coordinates = []
            for elem in sprinklers:
                sprinklers_coordinates.append((elem.coordinate_x(), elem.coordinate_y()))
            data = {"name1": "Игрок", "background_mark": background_mark,
                    "pos_x": player.get_x(), "pos_y": player.get_y(),
                    "inventory_picked": inventory.give_picked(),
                    "slot_1": wheat_seed.get_usage(), "slot_2": wheat_item.get_count(),
                    "slot_3": potato_seed.get_usage(), "slot_4": potato_item.get_count(),
                    "slot_5": milk_item.get_count(), "slot_6": sprinkler_item.get_count(), "slot_10": money.get_usage()}
    ...
    texts = [shop_text, wheat_seed_text, wheat_item_text, potato_item_text, potato_seed_text, money_text,
             milk_item_text, sprinkler_item_text]
    ...
    if not flag_background:
        ...
        if keys[pg.K_s]:
            player.move(dy=1)
        if keys[pg.K_e] and Inventory.give_picked(
                inventory) == 5 and sprinkler_item.get_count() > 0 and background_mark == 1 and iters > 90:
            flag = True
            for i in range(14):
            ...
            if flag:
                ...
                iters = 0
        if keys[pg.K_e] and iters > 30 and Inventory.give_picked(
                inventory) == 0 and wheat_seed.get_usage() > 0 and background_mark == 1:
            for i in range(14):
                if i == player.rect.centerx // 96:
                    x = i * 96 + 48
                    break
            ...
            iters = 0
        if keys[pg.K_e] and iters > 30 and Inventory.give_picked(
                inventory) == 2 and potato_seed.get_usage() > 0 and background_mark == 1:
            for i in range(14):
                if i == player.rect.centerx // 96:
                    x = i * 96 + 48
                    break
